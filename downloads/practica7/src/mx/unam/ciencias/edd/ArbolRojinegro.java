...
extends ArbolBinarioOrdenado<T> {
    ...
    private void rebalancea(VerticeRojinegro o){
        //Caso 1
        VerticeRojinegro p = getPadre(o);
        if (p==null) {
            o.color = Color.NEGRO;
            return;
        }
        ...
        if(esNegro(p))
            return;
        VerticeRojinegro t = getTio(o);
        VerticeRojinegro a = getAbuelo(o);
        //caso 3
        if (esRojo(t)) {
            p.color=Color.NEGRO;
            t.color= Color.NEGRO;
        ...
        //caso 4
        if(cruzadosIzq(o)|| cruzadosDer(o)){
            if (esIzquierdo(p))
                super.giraIzquierda(p);
            else
                super.giraDerecha(p);
            VerticeRojinegro aux = o;
            o = p;
            p = aux;
        }
        //Caso 5
    ...
    private VerticeRojinegro getAbuelo(VerticeRojinegro v){
        if (hayAbuelo(v))
            return convertirRojiNegro(getPadre(getPadre(v)));
        return null;
    }

    private boolean hayTio(VerticeRojinegro v){
        if(hayAbuelo(v)){
            if(esIzquierdo(getPadre(v)))
                return getAbuelo(v).derecho!=null;
            return getAbuelo(v).izquierdo != null;
        }
        return false;
    }
    ...
    private boolean hayHermano(VerticeRojinegro v){
        return tiene2hijos(getPadre(v));
    }

    private VerticeRojinegro getTio(VerticeRojinegro v){
        if (hayTio(v)) {
            if (esDerecho(convertirRojiNegro(v.padre)))
                return convertirRojiNegro(getAbuelo(v).izquierdo);
            VerticeRojinegro v1= getAbuelo(v);
